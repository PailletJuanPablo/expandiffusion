...
class StableDiffusionXLFillPipeline(DiffusionPipeline, StableDiffusionMixin):
    ...
    def __call__(
        ...
        control_mode = max(0, min(7, int(control_mode)))
        controlnet_image_list = [0, 0, 0, 0, 0, 0, 0, 0]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == 2:
                    ...
                    image = image[-1:]
                    for control_index, control_image in enumerate(controlnet_image_list):
                        if isinstance(control_image, torch.Tensor):
                            controlnet_image_list[control_index] = control_image[-1:]
